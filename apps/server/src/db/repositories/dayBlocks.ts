...
import type { BlockKind, DayBlock, Id, PracticeWeekday } from '@haeppi/shared';
...
export function listDayBlocks(db: Db): DayBlock[] {
  return (
    db.prepare(`SELECT * FROM day_blocks ORDER BY weekday, start_min`).all() as DayBlockRow[]
  ).map(toDayBlock);
}
...
export function findBlockConflict(blocks: readonly DayBlockInput[]): string | null {
  for (const weekday of PRACTICE_WEEKDAYS) {
    ...
    if (clash) {
      ...
      return (
        `Am selben Wochentag überschneiden sich zwei Blöcke: ` +
 ...
 * daran haengt.
 */
export function replaceDayBlocks(db: Db, blocks: readonly DayBlockInput[]): DayBlock[] {
  db.transaction(() => {
    const keep = new Set(blocks.map((block) => block.id).filter(Boolean) as string[]);
    const existing = db.prepare(`SELECT id FROM day_blocks`).all() as { id: string }[];
...
export function usageOfRemovedBlocks(db: Db, blocks: readonly DayBlockInput[]): DayBlockUsage[] {
  const keep = new Set(blocks.map((block) => block.id).filter(Boolean) as string[]);
  const usage: DayBlockUsage[] = [];

  for (const row of db.prepare(`SELECT id, label FROM day_blocks`).all() as {
    id: string;
    label: string;
  }[]) {
