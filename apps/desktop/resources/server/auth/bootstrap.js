 ...
 */
export async function ensureAdminAccount(db) {
    ...
    db.prepare(`INSERT INTO users (id, username, password_hash, role, must_change_password)
     VALUES (?, ?, ?, 'admin', 1)`).run(randomUUID(), username, await hashPassword(password));
    return { created: true, username, password };
}
export function printFirstRunNotice(result) {
