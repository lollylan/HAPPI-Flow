...
import { z } from 'zod';
import { assertIsoDate, calculateVacationBalance, closedDateSet, consumesVacationDays, } from '@haeppi/shared';
import { requireAdmin, requireAuth } from '../../auth/middleware.js';
import { countOpenRequests, createAbsence, createClosure, createRecurringAbsence, decideAbsence, deleteAbsence, deleteClosure, deleteRecurringAbsence, getAbsence, getVacationAccount, listAbsences, listAbsencesOfEmployee, listClosures, listRecurringAbsences, setVacationAccount, } from '../../db/repositories/absences.js';
...
const SELF_SERVICE_TYPES = ['vacation', 'sick'];
export function absencesRouter() {
    const router = Router();
    ...
    router.post('/', requireAuth, (req, res) => {
        ...
        if (!getEmployee(req.db, input.employeeId))
        ...
        const status = user.role === 'admin' || input.type === 'sick'
            ? 'approved'
            : 'requested';
    ...
    router.post('/:id/decide', requireAdmin, (req, res) => {
        ...
        if (!absence)
            throw notFound('Diese Abwesenheit gibt es nicht.');
    ...
    router.get('/vacation/:employeeId', requireAuth, (req, res) => {
        ...
        const approved = listAbsencesOfEmployee(req.db, employeeId, year).filter((absence) => absence.status === 'approved' && consumesVacationDays(absence.type));
        const balance = calculateVacationBalance({ entitlement: account.entitlement, carryover: account.carryover }, approved, employee.workTimes, closed);
...
export function closuresRouter() {
    const router = Router();
    router.get('/', requireAuth, (req, res) => {
    ...
    router.delete('/:id', requireAdmin, (req, res) => {
        if (!deleteClosure(req.db, pathParam(req, 'id')))
            throw notFound('Schließzeit nicht gefunden.');
        res.status(204).end();
    });
    return router;
}
export function recurringAbsencesRouter() {
    const router = Router();
    router.get('/', requireAuth, (req, res) => {
    ...
    router.delete('/:id', requireAdmin, (req, res) => {
        if (!deleteRecurringAbsence(req.db, pathParam(req, 'id')))
