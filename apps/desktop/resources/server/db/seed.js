...
const SEED_ROOMS = [1, 2, 3, 4].map((number) => ({
    id: `wa-zimmer-${number}`,
    plan: 'doctor',
    name: `Zimmer ${number}`,
...
export function isSeeded(db) {
    const row = db.prepare('SELECT COUNT(*) AS n FROM day_blocks').get();
    return row.n > 0;
}
/** Legt Zeitmodell, Qualifikationen, Arbeitsbereiche und Einstellungen an. */
export function seedDatabase(db) {
    const blocks = buildDayBlocks();
    const insertBlock = db.prepare(`INSERT INTO day_blocks (id, weekday, label, kind, start_min, end_min, sort_order)
     VALUES (@id, @weekday, @label, @kind, @startMin, @endMin, @sortOrder)`);
    const insertSkill = db.prepare(`INSERT INTO skills (id, name, category, description) VALUES (?, ?, ?, ?)`);
    const insertArea = db.prepare(`INSERT INTO work_areas
    ...
    const insertAreaSkill = db.prepare(`INSERT INTO work_area_skills (work_area_id, skill_id) VALUES (?, ?)`);
    const insertAreaBlock = db.prepare(`INSERT INTO work_area_blocks (work_area_id, day_block_id, min_staff) VALUES (?, ?, ?)`);
        ...
        for (const area of [...SEED_AREAS, ...SEED_ROOMS]) {
            ...
            for (const block of blocks) {
                if (block.kind === 'closed')
                    continue;
                if (!area.blockKinds.includes(block.kind))
                    continue;
                // Das Labor laeuft nur vormittags - sortOrder 1 ist der erste Block des Tages.
                if (area.morningsOnly && block.sortOrder !== 1)
                    continue;
