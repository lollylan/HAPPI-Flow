...
export function loadMigrations(dir = migrationsDirectory()) {
    return readdirSync(dir)
        ...
        if (!match?.[1])
            throw new Error(`Unerwarteter Migrationsname: ${file}`);
        return {
 ...
 */
export function runMigrations(db, dir = migrationsDirectory()) {
